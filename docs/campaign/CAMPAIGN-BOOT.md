...
**Resume point = U2 (move-speed-track).** U1 shipped (atk-speed). Run the per-unit loop on U2: re-ground vs HEAD → implement `Config.MOVESPD {PER_LEVEL .08, LEVELS 6, BASE 40, GROWTH 2.4}` + cached `moveSpeedMult()` in economy (mirror the U1 buy/cost/save/shop-row pattern) + multiply ACCEL/MAX_SPEED/walkPhase in entities.js update (locomotion ~:963-1027) → preview eval-drive (`moveSpeedMult()===1.08` at L1, no clipping) → 5-gate → tag `gz-u02-movespeed` → flip JSON → push. Cache still gz-v13 (no bump until U5 deploy). Local preview save was reset clean during U1.
