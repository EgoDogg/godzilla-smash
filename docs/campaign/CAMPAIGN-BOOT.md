...
**Resume point = U7 (pwa-hardening-trio).** v14 LIVE; U6 P0 fixed. Run U7 per the per-unit loop (apply-spec docs/audit-2026-06.md P1): (a) sw.js catch — serve `./index.html` only when `e.request.destination==='document'`, else return a 503 with `Content-Type: application/javascript` (stop serving HTML-as-JS on a partial cache); stop caching non-OK responses; (b) boot `console.warn` if `sw.js CACHE !== Config.CACHE_VERSION` (cross-comment both); (c) economy.js `save()` — check `U.safeSave` return, on first failure `G.Env.announce('Progress is not saving (storage full or private mode)')` ONCE (G.UI.toast does NOT exist — use Env.announce). Then U8 (save-system: version+lastSeen, dirty-flag save cadence), U9 (consolidations + DEPLOY gz-v15). Cache stays gz-v14 until U9. NOTE: SW changes need a cache bump to take effect on devices — but keep the bump for U9 (the v15 deploy); U7/U8 land in the same gz-v15 ship. Verify SW logic in preview via the fetch handler directly. Run the per-unit loop on U4: re-ground vs HEAD → implement the research-locked touch geometry in input.js (tunables ~:46-53 + layoutSmash): `JOY_DEADZONE 0.18→0.12`, `LEFT_ZONE 0.55→0.42` + delete dead `RIGHT_ZONE`, `SMASH_MIN 64→96`, `JUMP_MIN 56→76`, `FINISHER_MIN` already 76 (keep); keep `JOY_RADIUS 70`. **NEW safe-area insets:** in `layoutSmash` (~:151-152) the inset is flat `r+26` with NO `env(safe-area-inset)` — add `env(safe-area-inset-right)` to insetX and `env(safe-area-inset-bottom)` to insetY (hidden probe div + getComputedStyle; Canvas2D layout). Re-verify 3-disc clearance at the new sizes + 0.42 zone, both orientations. Tap-to-target now works center-screen (LEFT_ZONE 0.42). Apply-spec: docs/research-2026-06.md U4. → 5-gate → tag `gz-u04-touch` → flip JSON → push. Cache still gz-v13 (U5 = the gz-v14 deploy). Local preview save cleared at U3 close.
