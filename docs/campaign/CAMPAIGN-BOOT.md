...
**Resume point = U5 (v14-tuning + DEPLOY gz-v14) — the first LIVE deploy.** U1-U4 shipped (speed tracks + Nova Slam + final touch geometry). Run U5: apply research-locked balance in config.js — `COMBO {WINDOW_MS 1600→3250, STEP 0.04→0.12, MAX 2.0}`, `RESPAWN.RUBBLE_MS 6500→2800` AND `RUBBLE_PER_TIER 450→150` (both required — see research §rubble), hit-stop floor 40→60/cap 95 (world.js ~:509, `Math.min(95, 60 + Math.min(tier,18)*2)`). Then DEPLOY: bump `sw.js CACHE` + `js/config.js CACHE_VERSION` gz-v13→**gz-v14** (drift gate: must match), `node --check`, commit, tag `gz-u05-v14-deploy`, push, **poll live https://egodogg.github.io/godzilla-smash/sw.js until CACHE==='gz-v14'**, then dispatch the G1 playtest PushNotification. Apply-spec: docs/research-2026-06.md U5. Prior PRIORS fallback combo/rubble/hitstop numbers superseded by the locked values. Run the per-unit loop on U4: re-ground vs HEAD → implement the research-locked touch geometry in input.js (tunables ~:46-53 + layoutSmash): `JOY_DEADZONE 0.18→0.12`, `LEFT_ZONE 0.55→0.42` + delete dead `RIGHT_ZONE`, `SMASH_MIN 64→96`, `JUMP_MIN 56→76`, `FINISHER_MIN` already 76 (keep); keep `JOY_RADIUS 70`. **NEW safe-area insets:** in `layoutSmash` (~:151-152) the inset is flat `r+26` with NO `env(safe-area-inset)` — add `env(safe-area-inset-right)` to insetX and `env(safe-area-inset-bottom)` to insetY (hidden probe div + getComputedStyle; Canvas2D layout). Re-verify 3-disc clearance at the new sizes + 0.42 zone, both orientations. Tap-to-target now works center-screen (LEFT_ZONE 0.42). Apply-spec: docs/research-2026-06.md U4. → 5-gate → tag `gz-u04-touch` → flip JSON → push. Cache still gz-v13 (U5 = the gz-v14 deploy). Local preview save cleared at U3 close.
