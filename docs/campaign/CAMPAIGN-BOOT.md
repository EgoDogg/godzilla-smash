...
**Resume point = U3 (nova-slam-finisher) — HIGH-stakes, full verify panel.** U1+U2 shipped (both speed tracks). Run the per-unit loop on U3: re-ground vs HEAD → design → 3-lens adversarial verify → implement `Config.FINISHER {COST 350, CHARGE_S 1.2, COOLDOWN_S 8, RADIUS_T 2.5→3.2, DMG 3-10, MIN_CHARGE 0.15, SLOW 0.5, SHAKE 14}` + `fireFinisher()` parallel to startAttack (NEVER touches atkCooldown) + 3rd NOVA disc (76pt) LEFT of SMASH + F-key + intent.charge/chargeRelease (input.js) + charge/cooldown ring (render.js) + Audio.finisher + `finisherOwned` save field. Charge block in entities update multiplies spdMult ×FIN.SLOW. Apply-spec: docs/research-2026-06.md U3. → 5-gate (incl. both-orientation disc geometry + pointercancel=silent-cancel) → tag `gz-u03-novaslam` → flip JSON → push. Cache still gz-v13 (no bump until U5 deploy). NOTE: local preview save is polluted from U2 buy-testing — clear `godzilla-save-v3` during U3 verify.
