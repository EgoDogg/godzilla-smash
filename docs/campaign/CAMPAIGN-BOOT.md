...
**Resume point = U8 (save-system).** v14 LIVE; U6+U7 done. Run U8 per the per-unit loop (apply-spec docs/next-run-execution-plan.md gz-v15 + docs/audit-2026-06.md `economy-bankdestroy-save-every-kill`): add `version: 3` + a `lastSeen` timestamp to the save blob (lastSeen enables a future offline-income floor with no second migration); **dirty-flag the save cadence** — `bankDestroy` (economy.js) and the `maxReachedRow` setter currently hit localStorage on EVERY kill (twice on frontier kills); replace with a dirty flag flushed on a 2s timer + `visibilitychange`/`pagehide`, while keeping the IMMEDIATE save for purchases/setMuted (don't risk losing a paid upgrade). Verify: save JSON gains version+lastSeen; eval that rapid bankDestroy no longer calls setItem each time (spy on Storage.setItem); a flush fires on visibilitychange. Then U9 (consolidations C1/C2/C3/C6 + small guards + DEPLOY gz-v15 — the cache bump that ships U6/U7/U8/U9 to devices). Cache stays gz-v14 until U9. Run the per-unit loop on U4: re-ground vs HEAD → implement the research-locked touch geometry in input.js (tunables ~:46-53 + layoutSmash): `JOY_DEADZONE 0.18→0.12`, `LEFT_ZONE 0.55→0.42` + delete dead `RIGHT_ZONE`, `SMASH_MIN 64→96`, `JUMP_MIN 56→76`, `FINISHER_MIN` already 76 (keep); keep `JOY_RADIUS 70`. **NEW safe-area insets:** in `layoutSmash` (~:151-152) the inset is flat `r+26` with NO `env(safe-area-inset)` — add `env(safe-area-inset-right)` to insetX and `env(safe-area-inset-bottom)` to insetY (hidden probe div + getComputedStyle; Canvas2D layout). Re-verify 3-disc clearance at the new sizes + 0.42 zone, both orientations. Tap-to-target now works center-screen (LEFT_ZONE 0.42). Apply-spec: docs/research-2026-06.md U4. → 5-gate → tag `gz-u04-touch` → flip JSON → push. Cache still gz-v13 (U5 = the gz-v14 deploy). Local preview save cleared at U3 close.
